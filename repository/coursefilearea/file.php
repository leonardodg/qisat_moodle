...
if ($args[0] == 'blog') {
    ...
    if(!is_enrolled($context) || strtolower($args[1]) != 'biblioteca')
        require_login($course->id, true, null, false);

    if($bloqueio_acesso_extensao = $DB->get_record('config', ['name' => 'bloqueio_acesso_extensao'], 'value')){
        $bloqueio_acesso_extensao = explode(',', $bloqueio_acesso_extensao->value);
        foreach($bloqueio_acesso_extensao as $extensao){
            if($extensao == substr($pathname, strlen($extensao)*-1)){
                print_error('Arquivo Indisponível');
            }
        }
    }
    ...
    $dirroot = diretorioAcesso($relativepath);
    ...
    } elseif($dirroot != $CFG->coursefilearea_public_dir) {
        require_login(0, true, null, false);
    }
}
...
/**
 * Verify to the dir roo to access the file
 * @param string $urlAcesso url of access the file
 *
 * $return string name of dir root
 **/
function diretorioAcesso($urlAcesso){
    $pos = stripos($urlAcesso, '/');
    $str = substr($urlAcesso, $pos+1);
    $pos = stripos($str, '/');
