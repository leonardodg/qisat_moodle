...
/**
 * Returns the peiod to display for the certificate.
 *
 * @param stdClass $certificate
 * @param stdClass $certrecord
 * @param stdClass $course
 ...
 * @return string the date
 */
function certificate_get_period($certificate, $certrecord, $course, $userid = null)
{
    global $DB, $USER;
    ...
    $period = $DB->get_record_sql($sql);

    if(!isset($period->timestart)){
        $period = new stdClass();
        $period->timestart = time();
        $period->timeend = time();
    }

    if(isset($certrecord->timecreated)){
        $period->timeend = $certrecord->timecreated;
    }
 ...
 * @return string the date
 */
function certificate_get_presencial($course, $userid = null)
{
    global $DB, $USER;
    ...
    $sql = "SELECT MIN(datainicio) as datainicio, MAX(datafim) as datafim, 
                ci.nome as cidade, es.uf as estado FROM {groups} gr 
            INNER JOIN {groups_members} gm ON gm.groupid = gr.id 
            INNER JOIN ecm_curso_presencial_turma pt ON pt.id = gr.idnumber 
            INNER JOIN ecm_curso_presencial_data pd ON pd.ecm_curso_presencial_turma_id = pt.id 
            INNER JOIN ecm_curso_presencial_local pl ON pl.id = pd.ecm_curso_presencial_local_id 
            INNER JOIN {cidade} ci ON ci.id = pl.mdl_cidade_id 
            INNER JOIN {estado} es ON es.id = ci.uf 
            WHERE gr.courseid = $course->id AND gm.userid = $userid";
