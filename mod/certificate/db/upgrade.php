...
function xmldb_certificate_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2017010300) {
        ...
        $field = new xmldb_field('endereco', XMLDB_TYPE_CHAR, '255', null, false, 0, '');
        if (!$dbman->field_exists($table, $field))
            $dbman->add_field($table, $field);

        $field = new xmldb_field('nota', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, 0, '0');
        if (!$dbman->field_exists($table, $field))
            $dbman->add_field($table, $field);

        // Certificate savepoint reached.
        upgrade_mod_savepoint(true, 2017010300, 'certificate');
    }

    if ($oldversion < 2017010500) {
        // Fix previous upgrades.

        $table = new xmldb_table('certificate');

        $field = new xmldb_field('impresso', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, 0, '0');
        if (!$dbman->field_exists($table, $field))
            $dbman->add_field($table, $field);

        // Set impresso to certificatetype as A4_CertificadoQiSat
        $sql = "UPDATE {certificate}
                SET impresso = 1
                WHERE certificatetype LIKE 'A4_CertificadoQiSat'";
        $DB->execute($sql);

        // Certificate savepoint reached.
        upgrade_mod_savepoint(true, 2017010500, 'certificate');
    }

    if ($oldversion < 2017010900) {
        $table = new xmldb_table('certificate_issues');

        $field = new xmldb_field('solicitado', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, 0, '0');
        if (!$dbman->field_exists($table, $field))
            $dbman->add_field($table, $field);
        
        upgrade_mod_savepoint(true, 2017010900, 'certificate');
    }
