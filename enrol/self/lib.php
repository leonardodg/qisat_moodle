...
class enrol_self_plugin extends enrol_plugin {
     ...
     * - startnotifyhour plugin setting,
    ...
    public function send_start_notifications($trace) {
        ...
        $name = $this->get_name();
        $enablenotify = $this->get_config('enablenotifystart');
        ...
        $params = array('enabled'=>ENROL_INSTANCE_ENABLED, 'active'=>ENROL_USER_ACTIVE, 'data_notify'=> $timestart-86400, 'now' => $timestart,  'name'=>$name);
    ...
    protected function notify_start_enrolled($user, $ue, progress_trace $trace) {
        ...
        $a->timestart     = userdate($ue->timestart, get_string('strftimedatefullshort', 'core_langconfig'), $user->timezone);
        $a->firstday      = intval((time() - $ue->timestart) / 86400);
        $a->remainingdays = intval(($ue->timeend - time()) / 86400);
        $a->timeend       = userdate($ue->timeend, get_string('strftimedatefullshort', 'core_langconfig'), $user->timezone);
        
        //$a->enroller = fullname($enroller, has_capability('moodle/site:viewfullnames', $context, $user));

        $subject = get_string('startmessageenrolledsubject', 'enrol_self', $a);
        $body = get_string('startmessageenrolledbody', 'enrol_self', $a);
