...
$ntopicos = 0;// numero de tópicos
$ntopicosnaoassistidos = 0;// numero de tópicos	nao assistidos
global $ntopicos, $ntopicosnaoassistidos;
...
function build_mnet_logs_array($hostid, $course, $user=0, $date=0, $order="l.time ASC", $limitfrom='', $limitnum='',
    ...
    $result = array();
    $result['totalcount'] = $DB->count_records_sql("SELECT COUNT(*) FROM {mnet_log} l WHERE $where");
...
/// Next, have there been any modifications to the course structure?
    ...
    $logs = $DB->get_records_select('log', "time > $timestart AND course = $course->id AND
                                       module = 'course' AND
        ...
        foreach ($sectionmods as $modnumber) {
            ...
            if ($mod->modname == "label") {
				...
				if ( !$incomplete || !$locked ) { // allow link if module is not locked, or if module is completed
					...
					if ($section->section != 0){ //contar topicos
						global $ntopicos;
						$ntopicos++;
					}

					//verificar se foi assistido
					if (has_capability('moodle/course:showaccessed' , get_context_instance(CONTEXT_COURSE,$course->id), $USER->id, false)){ // se diferenciar topicos assistidos
						$sql = "SELECT id FROM {log} l WHERE l.userid = $USER->id AND l.course = $course->id AND l.module = 'resource' AND l.cmid = $mod->id AND l.action = 'view'";
						$result = $DB->get_records_sql($sql);
						$a = mysql_affected_rows();
						if (($section->section == 0) || ($a != 0)){
							$strong = '';
							$strongend = '';
						}else{
							global $ntopicosnaoassistidos;
							$ntopicosnaoassistidos++;
							$strong = '<strong>';
							$strongend = '</strong>';
						}
					}else{
					...
					echo '<a '.$linkcss.' '.$extra.        // Title unnecessary!
                         ...
                         '<img src="'.$icon.'" class="activityicon" alt="" /> <span>'.$strong.
                         $instancename.$altname.$strongend.'</span></a>';
                } else {
				...
				if (isediting() ) { // !isguestuser() ||
                    if ($locks and $mod->modname != "label") {
						if ($mod->checkboxesforprereqs) {
							foreach ($locks as $lock => $state) {
								$instance = $DB->get_record($mods[$lock]->modname, "id", $mods[$lock]->instance);
								if ($lock == "time") {
									if ($state == "closed") {
...
function rebuild_course_cache($courseid=0, $clearonly=false) {
    ...
    if ($rs = $DB->get_recordset_select("course", $select,'','id,fullname')) {
...
/// This function is only used by print_whole_category_list() above
    ...
    $courses = $DB->get_courses($category->id, 'c.sortorder ASC', 'c.id,c.sortorder,c.visible,c.fullname,c.shortname,c.password,c.summary,c.guest,c.cost,c.currency');
    if ($showcourses and $coursecount) {
...
function print_course_request_buttons($systemcontext) {
    ...
    if (has_capability('moodle/site:approvecourse', $systemcontext)) {
        print_single_button('pending.php', NULL, get_string('coursespending'), 'get', '_self', false, '', !$DB->record_exists('course_request'));
    }
}
...
function set_coursemodule_visible($id, $visible, $prevstateoverrides=false) {
    ...
    if ($events = $DB->get_records_select('event', "instance = '$cm->instance' AND modulename = '$modulename'")) {
...
/// Moves a whole course section up and down within the course
    ...
    for ($i = 0, $ref = $section + $direction; $i < $count; ++$i, $ref += $direction) {
        if (!$DB->set_field("course_sections", "section", $ref - $direction, array('course'=>$course->id, 'section'=>$ref))) {
            return false;
        }
    }
    ...
    $sections = $DB->get_records_select('course_sections', "course = $course->id", 'section ASC');
    $n = 0;
    foreach ($sections as $section) {
            ...
            foreach ($courseids as $courseid) {
                ...
                if (! $course  = $DB->get_record("course", array("id"=>$courseid))) {
                    ...
                    $sortorder = $DB->get_field_sql('SELECT MIN(sortorder)-1 AS min
                                                    FROM {course} WHERE category=' . $categoryid);
                    ...
                    if (!$DB->update_record('course', addslashes_recursive($course))) {
...
function can_delete_course($courseid) {
    ...
    return $DB->record_exists_select('log', $select);
}
...
function save_local_role_names($courseid, $data) {
    ...
    foreach ($data as $fieldname => $value) {
        ...
        if (!$value) {
            ...
            $rolename->name = $value;
            $DB->insert_record('role_names', $rolename, false);
        }
    }
}
...
function create_course($data) {
    ...
    fix_course_sortorder();
    $data->sortorder = $DB->get_field_sql("SELECT min(sortorder)-1 FROM {course} WHERE category=".$data->category);
