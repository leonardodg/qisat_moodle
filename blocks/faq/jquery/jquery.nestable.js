...
(function($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		addAcao : function(li) {
			...
			if (li.prev().index() >= 0) {
				if (li.attr('id').indexOf("artigo") == -1) {
					var lists = li.children(this.options.listNodeName);
					if (!li.attr('data-parent') && lists.length == 0)
						acao.children('#right').show();
				} else if (li.prev().attr('id').indexOf("artigo") == -1) {
					acao.children('#right').show();
				}
			}
		...
		edit : function(li) {
			...
			if (li.attr('id').indexOf("artigo") == -1) {
				...
				updateArtigo(li.attr('data-parent'), li.attr('data-id'));
		...
		salve : function(li) {
			...
			li.children('.dd-handle').html('Categoria: ' + texto);
			...
			updateCategoria(li.attr('data-id'), texto, li.attr('data-show'));
		...
		del : function(li) {
			...
			if (li.attr('id').indexOf("artigo") == -1) {
				deleteCategoria(li.attr('data-id'), parent.attr('data-id'));
			} else {
				deleteArtigo(li.attr('data-id'));
			}
		...
		show : function(li) {
			...
			if (li.attr('id').indexOf("artigo") == -1) {
				updateCategoria(li.attr('data-id'), li.attr('data-value'), 1);
			} else {
				updateArtigoVisible(li.attr('data-id'), 1);
			}
		},
		...
		hide : function(li) {
			...
			if (li.attr('id').indexOf("artigo") == -1) {
				updateCategoria(li.attr('data-id'), li.attr('data-value'), 0);
			} else {
				updateArtigoVisible(li.attr('data-id'), 0);
			}
		},
		...
		dragMove : function(e) {
			 ...
			 * move horizontal
			 */
			var li = this.dragEl.children(this.options.itemNodeName).first();
			if (mouse.dirAx && mouse.distAxX >= opt.threshold) {
				...
				if (mouse.distX > 0 && prev.length
					...
					// ********** Não permite a categoria ter uma terceira
					if (li.attr('id').indexOf("artigo") != -1) {
						depth--;
					}
	...
	$.fn.nestable = function(params) {
		...
		$.fn.nestable.addItem = function(value) {
			var id = insertCategoria(value);

			var num = lists.find(".dd-item").size();
			;
			var li = $("<li> </li>");
			li.addClass("dd-item").attr("data-id", id)
					.attr("id", "item_" + num).attr("data-value", value).attr(
							"data-show", "1").attr("data-ordem", num);
			var div = $("<div> </div").addClass("dd-handle").text(
					"Categoria: " + value);
