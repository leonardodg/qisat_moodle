...
class blocks_gerenciamento_graficoduvidasmensais_form extends moodleform {
	...
	function definition() {
		...
		$date = array();
		$date[0] = get_string('allanos', 'block_gerenciamento');
		for($cont = 2003; $cont <= date("Y", time()); $cont++){
			$date[$cont] = $cont;
		}
		$mform->addElement('select', 'ano', get_string('inicioem', 'block_gerenciamento'), $date);
