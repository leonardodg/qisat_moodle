...
require_once 'forms/alterarnome_form.php';
...
$PAGE->set_url('/blocks/gerenciamento/Central/AlunosAcessos/alterarnome.php');
$PAGE->set_title(get_string('alterarnome', 'block_gerenciamento'));
$PAGE->navigation->add(get_string('alterarnome', 'block_gerenciamento'));
...
$PAGE->navbar->add(get_string('blockname', 'block_gerenciamento'))->
	...
	add(get_string('alterarnome', 'block_gerenciamento'),
	new moodle_url('/blocks/gerenciamento/Central/AlunosAcessos/alterarnome.php', array('id'=>$id)));
...
if (has_capability('block/gerenciamento:alterarnome', $context)) {
	$form = new blocks_gerenciamento_alterarnome_form($CFG->wwwroot.'/blocks/gerenciamento/Central/AlunosAcessos/alterarnome.php',
		array('id'=>$id));

	$user = $DB->get_record('user', array('id' => $id), 'firstname,lastname,username', MUST_EXIST);
	...
	if ($form->is_cancelled()) {
		...
		/**
		 * Email de alteração de nome de aluno
		 */
		...
		$fromemail->nome2 = $data->firstname . " " . $data->lastname;
		$fromemail->nomeusuario = $USER->firstname . " " . $USER->lastname;
		$fromemail->data = date('d/m/Y - H:i:s');

		$emailsubject = get_string('alteracaonome', 'block_gerenciamento');
		$emailbody = get_string('alteracaonomeadmin', 'block_gerenciamento', $fromemail);
		$admin = get_admin();
		$admin->mailformat = 1;
