...
$chave   = required_param('chave', PARAM_TEXT); // user enrolment id
...
$PAGE->set_url('/blocks/gerenciamento/Central/AlunosAcessos/tabelas/unenroluser.php', array('ue'=>$ueid, 'chave'=>$chave, 'ifilter'=>$filter));
...
$returnurl = new moodle_url('../consultageral.php', array('chave' => $chave));
$usersurl = new moodle_url('../consultageral.php', array('chave' => $chave));
...
if ($confirm && confirm_sesskey()) {
    ...
    $emailsubject = get_string('cursoexcluido', 'block_gerenciamento');
    $emailbody = get_string('cursoexcluidoadmin', 'block_gerenciamento', $fromemail);
    $admin = get_admin();
    $admin->mailformat = 1;
